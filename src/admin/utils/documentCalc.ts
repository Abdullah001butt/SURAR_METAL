...
export function calcTotals(items: DocumentItem[], discount: number, vatRate: number, manualTotal?: number | null) {
  // A manually-typed amount (e.g. a handwritten lump-sum quote) wins over the
  // item-based total when set — items may just describe scope of work without
  // individual pricing (qty/unit filled in, unit price left at 0). The manual
  // amount is treated as the pre-VAT (gross) amount, same as the item-based
  // gross would be, so VAT is always added on top consistently either way:
  // e.g. AED 17,000 gross + 5% VAT = AED 850 VAT = AED 17,850 net total.
  const gross = manualTotal != null ? manualTotal : items.reduce((sum, item) => sum + itemAmount(item), 0)
