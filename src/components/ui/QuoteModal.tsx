...
export function QuoteModal({ open, onClose, variant = 'default' }: QuoteModalProps) {
  ...
  useLayoutEffect(() => {
    ...
    if (open) {
      ...
      gsap.fromTo(panel, { opacity: 0, y: 24, scale: 0.97 }, { opacity: 1, y: 0, scale: 1, duration: 0.45, ease: 'back.out(1.4)' })
    } else {
      gsap.to(overlay, { opacity: 0, duration: 0.25 })
      gsap.to(panel, { opacity: 0, y: 24, scale: 0.97, duration: 0.25, ease: 'power2.in', onComplete: () => setRendered(false) })
    }
  }, [open, rendered])
